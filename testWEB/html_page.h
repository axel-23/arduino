...
<head>
    ...
    <style>
        ...
        /* Estilo para el botón de mostrar */
        .show-btn {
            margin-left: 10px;
            padding: 5px;
    ...
    <script>
            ...
            fetch('/files')
                ...
                .then(files => {
                    ...
                    const html = files.map(file => `
                                            <li class="preview">
                                                <img src="/view?file=${file.name}" alt="${file.name}">
                                                ${file.name} - ${formatSize(file.size)}
                                                <button class="delete-btn" onclick="deleteFile('${file.name}')">Eliminar</button>
                                                <button class="show-btn" onclick="showFile('${file.name}')">Mostrar</button>
                                            </li>
                                            `).join('');
        ...
        // Función para mostar un archivo usando FormData
        function showFile(filename) {
            ...
            fetch('/show', {
                ...
                .then(data => {
                    ...
                    loadFiles();  // Recargar la lista de archivos después de mostrar
                })
                .catch(error => {
        ...
        function deleteFile(filename) {
            ...
            // Agregar el nombre del archivo al FormData
            formData.append('fileDelete', filename);
